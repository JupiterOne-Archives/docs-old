...
- In Alerts -> Vulnerability Findings view, "Severity" and other fields are now
  **normalized** when filtering findings (users no longer need to select "Low",
  "LOW", and "low" in order to search for all findings with a low severity,
  instead there is just now a single "Low" to select).
