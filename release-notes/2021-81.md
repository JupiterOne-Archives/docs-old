...
- You can now view multiple assets in the entity drawer if you used multiple 
  assets for constructing a query result.
- Whitelist is renamed as allowlist in the resource management powerup.
...
Additional permissions are needed to ingest new Azure data:

- Fetching Azure app auth settings requires that you add additional permissions
     to the custom role `Microsoft.Web/sites/config/list/Action`.
...
- Fetching the `github_issue` entity requires the `issues:read` permission to be granted to the JupiterOne GitHub app by your GitHub Organization admin.  This data will not be imported if the permission is not granted, but other GitHub entities will continue to be ingested.
- Made enhancements to increase the ingestion performance of 
  various entities.
- Performed various fixes to improve ingestion consistency and 
  user job log error messaging
...
`Find google_user ASSIGNED google_iam_role` must change to `Find google_user that ASSIGNED google_iam_binding 
