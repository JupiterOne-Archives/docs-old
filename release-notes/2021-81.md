...
J1 recently discovered some ineffective relationships were being created between the `google_iam_role` and several entities, 
which may be causing the reporting of incorrect permissions in queries.

To correct this issue, a `google_iam_binding` entity is added to the Google Cloud integration that better represents how 
permissions are associated with principals in Google Cloud.
...
As a result of these changes, be aware of the following:
...
- There is a new [blog post](https://try.jupiterone.com/blog/google-cloud-storage-access-analysis) from J1 Engineering 
  that provides more details about these changes.
