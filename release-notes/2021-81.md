...
#### GitHub Analyze Commit Approval Changes
 ...
 When the GitHub integration was initially built there was a configuration option to enable Analyze Commit Approvals. This feature gives customers the ability to see which users have opened, reviewed and/or approved a pull request in their GitHub account.
 ...
 The `analyzeCommitApproval` step in the integration generates the following relationships in the graph:
  ...
  These entities appear in the graph, showing which users opened, reviewed and approved a pull request in GitHub:

  ![](../assets/graph-github-pullreques-data-model.png)

 Initially this was an optional setting because in large GitHub accounts it could result in API throttling for a customer’s GitHub account.  The integration team has recently found a way to include this information as part of the data imported to JupiterOne that does not have an adverse affect on a customer’s API usage. Because of this improvement to the integration, this data is now included in all GitHub integrations regardless of the setting in the configuration UI.

 Since it is no longer relevant, a forthcoming update will remove this setting from the UI.

 **Customer Impact**

 For customers that already had it enabled on their GitHub configurations, it will continue to behave the same after the UI is updated.

 For customers that did not enable it due to concerns of API throttling, they will now have additional GitHub data available in their JupiterOne account with no adverse affect on their GitHub API usage. 

 The option to ingest pull requests also exists on the Bitbucket integration.  There is no change in its behavior.  It is still an option that a customer can choose to enable or not.
