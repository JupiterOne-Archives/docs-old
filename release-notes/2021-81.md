...
- Fetching the `github_issue` entity requires the `issues:read` permission to be granted to the JupiterOne GitHub app by your GitHub  Organization admin.  This data will not be imported if the permission is not granted, but other GitHub entities will continue to ingested.
