...
- The query policy editor in the Account Settings page is improved to support the new array-based data structure recently changed in the JupiterOne APIs.
- Improved the query policy editor for user group permissions to show that data from ANY permission filter now returns results for users within the group.
- Updated the Account Usage graph to remove the Relationship Count label from the legend as this data is not supported.
...
- Resolved deleting a policy or procedure with an invalid template.
